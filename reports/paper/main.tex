...
\begin{abstract}
This paper examines whether financial statement variables can help distinguish
failed from alive company-year observations. The analysis focuses on
classification and does not make causal claims or test a clearly defined future
prediction period. The data contain
78,682 observations from 8,971 American companies between 1999 and 2018. Because
firms appear in several years, the split assigns complete companies to the
training and test samples.
The comparison includes a majority-class baseline, Logistic Regression and
regularized Logistic Regression variants, a Decision Tree, Random Forest,
Gradient Boosting, and a PCA plus Logistic Regression extension. Because
failures are rare, evaluation focuses on the failed class. Accuracy alone is
misleading: the majority baseline reaches 92.5\% final-test accuracy but detects
no failures. Ranking performance is evaluated mainly with PR-AUC, which is
calculated as average precision in this project. Failed-class recall and F1 are evaluated at the default
prediction rule, while threshold scenarios use validation predictions only.
Logistic Regression has the highest failed recall among the key models but
produces many false alarms. Random Forest has the numerically highest observed
final-test PR-AUC and failed-class F1. Gradient Boosting has the highest
balanced accuracy and was selected before the final test was examined because it
had the highest validation PR-AUC. PCA retains
much of the predictor variance but does not clearly improve validation
performance. Overall, the models provide useful information for screening
companies. However, the results are not causal, the models are not ready for
real financial decisions, and no model performs best for every objective.
\end{abstract}
...
errors, threshold trade-offs, coefficients, and feature importance. The results
are interpreted as predictive associations rather than causal effects. The
project is intended as an educational model comparison and not as a system for
automatic credit or investment decisions.
...
The validation rows are not added back for a final refit. This means that the
final models are fitted with fewer observations than they could have used. It is also less
data-efficient than refitting the selected specification on all non-test
training rows. That alternative was not used here, and this point is considered
...
model complexity without requiring a very large search. A compact specification
table is provided in Appendix~\ref{app:model-results}.
...
These rules show what happens when failure detection receives more weight.
...
failed recall at 0.884, but its failed precision is only 0.095. Random Forest
has the numerically highest observed final-test PR-AUC among the models shown
at 0.159 and the highest failed-class F1-score at 0.232. This result was
observed on the final test set and does not change the earlier model-selection
decision. Gradient Boosting has the highest
balanced accuracy at 0.638 and an intermediate recall of 0.590. The ROC-AUC
values of the three models are close, ranging from 0.688 to 0.699. Overall, the
models perform differently depending on the metric being considered.
...
total variance, five retain 93.9\%, and ten retain 99.2\%. The full
explained-variance curve is provided in Appendix~\ref{app:pca}.
...
The project includes several choices that strengthen the analysis. Splitting by company prevents the
same firm from appearing in both training and test data. Keeping the test set
separate from model selection and threshold analysis also makes the final
comparison more reliable. Finally, the majority baseline and failed-class
metrics expose weaknesses that would be hidden by accuracy. These choices
improve the analysis, but they do not remove the following limitations.
...
using explicit financial costs and review capacity. These improvements would be
more useful than simply adding another complex model.
...
Table~\ref{tab:model-settings} summarizes the model settings used in the
comparison.
...
regularized Logistic Regression models produce results that are very close to
the main Logistic Regression model. The Decision Tree has higher failed-class
recall than Random Forest, but its ranking performance is weaker and it
produces more false positives. The source is
...
Figure~\ref{fig:test-roc} shows the ROC curves for the three main models as an
additional comparison. ROC curves compare how well the models rank observations
across different thresholds, while the diagonal line represents random
classification.
...
These measures show the false-alarm burden more clearly than the false-positive
rate alone.
...
Table~\ref{tab:selected-thresholds} shows the validation-based threshold
...
The large increase in this share under the 80\% recall rule shows that a high
failure-detection target would require many more cases to be reviewed. These
are validation-based examples only and were not used to change the final test
comparison. Figure~\ref{fig:threshold-tradeoff} shows how validation precision,
recall, and failed-class F1 change across the tested thresholds.
...
Figure~\ref{fig:pca-variance} shows how much of the variation in the
standardized predictors is explained as more principal components are added.
...
Figure~\ref{fig:pca-metric-comparison} adds to the PCA results in
Section~\ref{sec:pca-results}. It shows that classification performance
improves as more components are added and then changes only slightly. The full numerical
...
The simplified academic repository uses a six-notebook workflow and fixed
random seeds. Paths, feature names, split settings, model grids, validation
logic, threshold analysis, PCA, interpretation, and paper-output generation are
visible inside the numbered notebooks. The notebooks are executed from
\texttt{01\_data\_audit\_and\_preparation.ipynb} through
\texttt{06\_generate\_paper\_assets.ipynb}. Visible assertions in the notebooks
check the main validity conditions, including target encoding, predictor
definition, company-level split disjointness, model names, probability bounds,
confusion-matrix consistency, threshold validity, PCA component validity, and
paper-output existence.

From the repository root, the complete notebook workflow can be run with:
...
The notebooks create the processed data, result tables, and figures in the
correct order. Generated analysis artifacts are kept under
\texttt{data/processed/} and \texttt{outputs/}. The \texttt{outputs/paper/}
folder contains simplified paper-ready tables and figures generated from the
canonical output files. Notebook~06 does not train models or change analytical
results.

The main result files used in the paper are listed below. Keeping these tables
with the figures makes it possible to trace each reported value back to a saved
project output.
